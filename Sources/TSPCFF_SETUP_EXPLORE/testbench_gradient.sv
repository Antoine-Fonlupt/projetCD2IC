...
// The following variables are defined for the gradient descent
...
localparam real capa_values[0:NBCAPA-1] = '{11}; // fF We limit to only one capacitor value for this test
...
logic dout_ref ;
// At each clock edge, we store the value of din into dout_ref
always @(posedge clk)
    dout_ref <= din ;
...
begin:simu
   ...
   begin
     ...
     // Secondary loop on the output load capacitor (This loops only once here)
     for(capa_index=0;capa_index<NBCAPA;capa_index++) 
     begin

      // Add 4th loop here (Instead of looping over clk_delay_val_index, we use gradient descent)
      clk_delay_val = 100.0*1e-12;
      // Convergence criterion
      while(((xn-xn_old)>0.2e-12) || ((xn-xn_old)<-0.2e-12))
      begin
        clk_delay_val = xn; // Update clk_delay_val with the current xn value
        load_capacitor_val = capa_values[capa_index]*1.0e-15  ; // This loop only once here
        // Apply the test sequence:
        // Clck once to store a 0
        // High clock
        // Wait
        // Low clock 
        #(digital_tick) ;
        ...
        clk = 1'b0 ;
        #(digital_tick) ;
        // Rise din and clk once to store a 1
        din = 1'b1 ;
        clk = 1'b1 ;
        #(digital_tick) ;
        // Wait for stabiity and fall clk and input
        clk = 1'b0 ;
        din = 1'b0 ;
        // For gradiant descent, we need to evaluate f(xn)
        f_xn = propagation_time + clk_delay_val;
        //Update clk_delay_val to xn+h
        clk_delay_val = xn+h;
        // Same test sequence
        ...
        // Update xn using gradient descent formula
        xn_old = xn;
